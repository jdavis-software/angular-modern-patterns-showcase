...
@Component({
  ...
  template: `
    <div class="performance-lab-demo">
      <h2>
        <lucide-icon [img]="BarChart3Icon" size="24"></lucide-icon> Performance
        Lab: Render Strategies at Scale
      </h2>
        ...
        <div class="test-results">
          <div class="results-header">
            ...
            <div class="results-summary">
              ...
              >
              <span *ngIf="bestStrategy()"
                >Best: {{ bestStrategy()?.name }}</span
              >
            </div>
          </div>
      ...
      <div class="demo-section">
        ...
        <div class="performance-monitor">
          <div class="monitor-controls">
            ...
            <button (click)="triggerStressTest()" class="btn-danger">
              Stress Test
            </button>
          </div>
          ...
          <div class="metrics-dashboard">
            ...
            <div class="metric-card">
              ...
              <div class="metric-info">
                <div class="metric-value">
                  {{ currentMetrics().changeDetectionCycles }}
                </div>
      ...
      <div class="performance-info">
        ...
        <ul>
          <li>
            <strong>OnPush Strategy:</strong> Reduces change detection cycles by
            60-90%
          </li>
          <li>
            <strong>Signals:</strong> Fine-grained reactivity with automatic
            optimization
          </li>
          <li>
            <strong>Virtual Scrolling:</strong> Handles 100K+ items with
            constant performance
          </li>
          <li>
            <strong>TrackBy Functions:</strong> Prevents unnecessary DOM
            manipulation
          </li>
          <li>
            <strong>Lazy Loading:</strong> Defers non-critical component
            initialization
          </li>
          <li>
            <strong>Memoization:</strong> Caches expensive computations and
            renders
          </li>
          <li>
            <strong>Web Workers:</strong> Offloads heavy processing from main
            thread
          </li>
          <li>
            <strong>Bundle Splitting:</strong> Reduces initial load time and
            memory usage
          </li>
        </ul>
      </div>
...
export class PerformanceLabDemoComponent
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  renderStrategies = signal<RenderStrategy[]>([
    {
      ...
      name: "Default Change Detection",
      description:
        "Angular's default change detection runs on every tick, checking all components.",
      pros: [
        "Simple to implement",
    ...
    },
    {
      ...
      description:
        "Only runs change detection when inputs change or events are triggered.",
      pros: [
    ...
    },
    {
      ...
      description:
        "Fine-grained reactivity that only updates when specific signals change.",
      pros: [
    ...
    },
    {
      ...
      name: "Virtual Scrolling",
      description:
        "Only renders visible items, handling massive datasets efficiently.",
      pros: [
        "Handles 100K+ items",
        "Constant performance",
  ...
  private async executePerformanceTest(test: PerformanceTest): Promise<void> {
    return new Promise((resolve) => {
      ...
      const renderTime = Math.round(
        baseTime * multiplier * (test.itemCount / 1000)
      );
      const memoryUsage = Math.round(
        test.itemCount * 0.1 * multiplier + Math.random() * 10
      );
  ...
  private updateMetrics() {
    ...
    const multiplier =
      strategyMultipliers[strategy as keyof typeof strategyMultipliers] || 1.0;
    const simulatedRenderTime = Math.round(
      baseRenderTime * multiplier + Math.random() * 5
    );

    // Simulate FPS impact from render time
    const simulatedFps = Math.max(
      15,
      Math.min(60, 60 - Math.floor(simulatedRenderTime / 2))
    );
  ...
  getPerformanceLevel(): string {
    ...
    if (metrics.renderTime <= adjustedExpectedTime && metrics.fpsAverage >= 55)
      return "excellent";
    if (
      metrics.renderTime <= adjustedExpectedTime * 1.5 &&
      metrics.fpsAverage >= 45
    )
      return "good";
    if (
      metrics.renderTime <= adjustedExpectedTime * 2.5 &&
      metrics.fpsAverage >= 30
    )
      return "fair";
    return "poor";
  }
  ...
  private drawChart() {
    ...
    const maxRenderTime = Math.max(
      ...this.chartData.map((d) => d.renderTime),
      100
    );
    ...
    this.chartData.forEach((point, index) => {
      ...
      const y =
        height - padding - (point.renderTime / maxRenderTime) * chartHeight;
